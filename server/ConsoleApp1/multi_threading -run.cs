...
/*
 * 서브스레드를 기반으로 한 경주 프로그램
 * 기능
 * 메인스레드는 서브스레드가 모두 종료할 때 까지 도달지점을 화면에 출력
...
namespace server
{
    ...
    class Player
    {
        //정적변수로 순위를 저장하는 배열과 인덱스 저장 변수 선언
        ...
        public void run(object seed)
        {
            ...
            for(distance = 0; distance < 500 ; distance += random.Next(10))
            {
                Thread.Sleep(50);
            }
            isFinish = true;
            //도착 후 순위 등록
            //lock(변수) : 특정 변수를 접근하는 다양한 스레드들의 데이터 입력/변경을 순차적으로 진행하도록 처리하는 문법(서브스레드를 사용할 때 필요한 제어)
            //ex) 1,5번 스레드가 거의 동시에 rank변수 접근하는 경우 가장먼저 lock 문법에 도착한 스레드가 점유하는 동안 다른 스레드가 대기상태로 정지함
            lock (rank)
            { 
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for(; ; )
            {
                ...
                for(int i = 0; i < num; i++)
                {
                    Console.WriteLine("{0} : {1}",players[i].number, players[i].distance);
                }
                Thread.Sleep(100);
            }
            //순위출력
