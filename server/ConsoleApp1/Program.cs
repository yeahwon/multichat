 ...
 * 기능
 * 메인스레드
 * 서버설정, 클라이언트 접속 대기, 접속한 클라이언트를 매개변수로 서브스레드 생성
 * 서브스레드
 * 클라이언트가 송신한 채팅내용을 다른 클라이언트들에게 송신
 * 서브스레드2..n
...
namespace server
{
    ...
    class User
    {
        ...
        //서브스레드 - 클라이언트가 송신한 데이터를 수신받아 메세지 저장 변수에 누적
        public void RecvData()
        {
            ...
            catch //ID값 수신 중 클라이언트가 연결 종료
            {
                Console.WriteLine("ID값 수신 대기중 연결 종료 확인");
                Close();
                return;
            }
            for (; ; )
            {
                try
                {
                    ...
                    //문자열을 메세지 저장변수에 전달
                    //여러 서브스레드가 요소를 동시에 추가하는 것을 막기 위해 lock 사용
                    lock (Manager.messages)
                    {
                        Manager.messages.Add(chat);
                    }
                }
                catch //데이터 수신 대기중 클라이언트가 연결을 끊었을 때의 예외처리
                {
                    Console.WriteLine("수신대기 중 연결된 클라이언트가 연결을 끊음");
                    break;
                }
            }
            Close();
    ...
    {
        //상수변수로 욕 등록
        const string FILTER = "바보"; //상수변수는 보편적으로 대문자 변수이름
        ...
        public void send_users()
        {
            ...
            for (; ; )
            {
                //조건문 - message에 보낼 채팅이 저장됐는지 확인
                if (messages.Count > 0)
                {
                    lock (messages) //messages 변수 접근을 하나의 스레드만 할 수 있도록 설정
                    {
                        chat = messages[0];
                        messages.RemoveAt(0);
                    }
                    ...
                    for (int i = 0; i < users.Count; i++)
                    {
                        ...
                        //연결이 끊긴 클라이언트를 리스트 객체에서 제거
                        if (!isConnected)
                        {
                            Console.WriteLine("{0}번째 클라이언트 리스트 제거", i);
                            user_tasks.RemoveAt(i);
                            users.RemoveAt(i);
                            i--; //지워진 요소만큼 i값을 뺌. 삭제된 인덱스 다음 요소가 데이터를 전송받도록 처리
                        }
                    }
                }
            }
        ...
        class Program
        {
            ...
            //connect_client 메소드에 무한반복이 있기 때문에 서버닫는 메소드가 호출되지 않음
