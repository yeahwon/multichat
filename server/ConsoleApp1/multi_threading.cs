...
namespace server
{
    ...
    class Program
    {
        //생성된 (또는 생성 할) 서브스레드를 강제종료 할 수 있는 토큰을 생성하는 클래스
        ...
        static void Main(string[] args)
        {
            ...
            //재생할 파일명을 사용자입력으로 받기
            Console.Write("실행할 파일의 이름을 입력(확장자명 포함) : ");
            string filename = Console.ReadLine();
            //서브스레드를 생성하면서 파일명 전달
            //서브스레드 종료를 위한 토큰 전달
            //전달된 토큰이 이미 취소명령이 내려진 상태면 서브스레드가 시작하지 않음
            Task task = new Task(new Action<object>(play_music), filename, source.Token);
            task.Start();
            Console.WriteLine("음악재생중. 종료하려면 엔터키 입력");
            Console.ReadLine(); //엔터키 입력 대기. source객체의 토큰을 가진 모든 서브스레드 취소명령 호출
            source.Cancel();
            //무한반복
            /*
            for(; ; )
            {
                Console.WriteLine("메인스레드 대기");
                Thread.Sleep(5000);
            }
            */
