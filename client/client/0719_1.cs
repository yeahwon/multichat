...
namespace client
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //Udp소켓 종료 
            socket.Close();
            recv_task.Wait(); //서브스레드가 종료할 때 까지 대기
        }
        //데이터 수신을 받기 위한 서브스레드 생성용 함수 정의
        //socket : UdpClient 객체
        static void recv_thread(object socket)
        {
            //서브스레드를 생성한 곳에서 매개변수를 안줬을 때에 대한 처리
            if(socket == null)
            {
            ...
            byte[] recv_data = null; // try-catch로 감싼 변수는 초기화 시켜주야 ㅎ람 !
            string result;
            BinaryFormatter formatter = new BinaryFormatter();
            MemoryStream ms = new MemoryStream();
            ...
            for(; !source.IsCancellationRequested ; )
            {
                ...
                catch //UdpClient객체가 Close되어 수신받지 못하는 예외처리
                {
