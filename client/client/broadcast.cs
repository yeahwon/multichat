...
/*
 * 브로드캐스트 : 해당 컴퓨터가 속한 네트워크 영역에 연결된 모든 컴퓨터/스마트폰/인터넷장비에게 동일한 데이터를 동일한 포트로 송신하는 기능
...
namespace client
{
    class Program
    {
        ...
        {
            //UdpClient 객체 생성
            UdpClient sender = new UdpClient(); // 보내는 udp는 포트를 고정할 필요 없고, 무작위로 발급받으면 
            //sender.EnableBroadcast : 해당 UDP소켓이 브로드캐스트를 사용하는지 설정하는 변수
            //true(기본값) : 브로드캐스트 허용
            //false : 브로드캐스트로 송신하거나 수신하지 못하도록 설정

            //IPEndPoint객체 생성 - 브로드캐스트 설정 및 12000번 포트로 설정
            ...
            for (; ; )
            {
                //보낼 데이터를 저장한 문자열 변수 선언 및 초기화
                Console.WriteLine("채팅입력 :");
